...
esp_err_t enable_motor_driver_a(int mode)
{
    ...
    {
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0A, AIN_1), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0B, AIN_2), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
    }
...
esp_err_t enable_motor_driver_b(int mode)
{
    ...
    if (mode == PARALLEL_MODE)
    {
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM1A, BIN_1), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM1B, BIN_2), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
    }
    else if (mode == NORMAL_MODE)
    {
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM1A, BIN_1), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
        CHECK_LOGE(err, mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM1tB, BIN_2), TAG_MOTOR_DRIVER, "error: %s", esp_err_to_name(err));
    }
    else
    {
        ESP_LOGE(TAG_MOTOR_DRIVER, "error: %s", "invalid motor driver mode selected");
    ...
    if (mode == PARALLEL_MODE)
    {
        ...
        esp_err_t err1_2 = mcpwm_init(MCPWM_UNIT_0, MCPWM_TIMER_2, &pwm_config);
...
esp_err_t set_motor_speed(int motor_id, int direction, float duty_cycle)
{
    ...
    else if ((motor_id == MOTOR_B_0 || motor_id == MOTOR_B_1) && read_motor_driver_mode(b) != 0)
    {
        ...
        {
            return set_motor_speed_helper(direction, duty_cycle, MCPWM_UNIT_0, MCPWM_TIMER_1);
        }
        else if (read_motor_driver_mode(b) == NORMAL_MODE)
        {
            mcpwm_unit_t unit_val = motor_id == MOTOR_B_0 ? MCPWM_UNIT_0 : MCPWM_UNIT_0;
